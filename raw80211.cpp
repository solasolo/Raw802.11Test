...
const int Raw80211::HEADER_LENGTH = 24;

uint8_t DefauleBSSID[] = {
    'e',
    's',
    's',
    'i',
...
void wifi_sniffer_packet_handler(void *buff, wifi_promiscuous_pkt_type_t type)
{
  ...
  /*
  if (pt->payload[0] != 0x40 || memcmp(this->_bssid, hdr->addr3, 6) != 0)
    return;
  */

  // Extract payload length
  unsigned char *d = (unsigned char *)pt->payload + Raw80211::HEADER_LENGTH;
...
// Output
  ...
  Serial.printf(" (RSSI: %d, Bandwidth; %d, Mode: %d, Rate: %d, Length: %d)\n", pt->rx_ctrl.rssi, pt->rx_ctrl.cwb, pt->rx_ctrl.sig_mode, pt->rx_ctrl.rate, len);
// dumphex((const uint8_t*)data + Raw80211::HEADER_LENGTH+2, len, "  ");
#endif

  Raw80211::_receive_callback(hdr, pt->rx_ctrl.rssi, (const uint8_t *)data + Raw80211::HEADER_LENGTH + 2, len);
}

Raw80211::Raw80211(uint8_t channel)
  : RawChannel(channel)
{
  memcpy(this->_raw_header, DefaultHeader, Raw80211::HEADER_LENGTH);
}
 ...
 */
void Raw80211::setCallback(RAW_CB cb)
{
...
{
  uint8_t buf[len + Raw80211::HEADER_LENGTH + 2];

  if (len > 1500)
    return;

  memcpy(buf, this->_raw_header, Raw80211::HEADER_LENGTH); // Copy raw header
  memcpy(buf + Raw80211::HEADER_LENGTH + 2, data, len);        // Copy payload data
  buf[Raw80211::HEADER_LENGTH] = (len >> 8) & 0xff;            // Copy length
  buf[Raw80211::HEADER_LENGTH + 1] = len & 0xff;
  ...
  Serial.printf(" (Length: %d)\n", Raw80211::HEADER_LENGTH + len + 2);
  // dumphex((const uint8_t*)buf, Raw80211::HEADER_LENGTH + len + 2, "  ");
#else
  Serial.printf(" (Length: %d)\n", len);
// dumphex((const uint8_t*)buf + Raw80211::HEADER_LENGTH + 2, len, "  ");
#endif
#endif
...
#ifdef ESP32
    esp_wifi_80211_tx(WIFI_IF_STA, buf, Raw80211::HEADER_LENGTH + len + 2, true);
#else
    wifi_send_pkt_freedom(buf, Raw80211::HEADER_LENGTH + len + 2, true);
#endif
  }
}
...
void Raw80211::Start()
{
  ...
  memcpy(this->_raw_header + SRC_MAC_OFFSET, mac, 6);
  memcpy(this->_raw_header + BSS_MAC_OFFSET, this->_bssid, 6);
